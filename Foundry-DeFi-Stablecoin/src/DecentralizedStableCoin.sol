...
contract DecentralizedStableCoin is ERC20Burnable, Ownable {
    ...
    /**
     * @notice Funcion burn(): Diseñada para permitir que el propietario del contrato (CSCEngine) queme una cierta
     * cantidad de tokens, siempre y cuando cumpla con las verificaciones de límite y tenga suficientes tokens en su
     * saldo.
     * @dev if() statement 1: Verifica si la cantidad que se quiere quemar es menor o igual a cero. Si es así, revierte
     * la transacción con un mensaje de error indicando que la cantidad debe ser mayor que cero.
     * @dev if() statement 2: Verifica si la cantidad que se quiere quemar es mayor que el saldo disponible en la cuenta
     * del propietario. Si es así, revierte la transacción con un mensaje de error indicando que la cantidad a quema
     * excede el saldo.
    ...
    /**
     * @notice Funcion mint(): Diseñada para permitir que el propietario del contrato (CSCEngine) cree nuevas unidades
     * de tokens y las asigne a una dirección específica, siempre y cuando se cumplan las condiciones de validación.
     * @dev if() statement 1: Verifica si la dirección de destino (_to) es la dirección 0. Revierte la transacción si
     * es así, con un mensaje de error indicando que la dirección no puede ser la dirección 0.
     * @dev if() statement 2: Verifica si la cantidad de tokens que se quiere crear y asignar es menor o igual a cero.
     * Revierte la transacción si es así, con un mensaje de error indicando que la cantidad debe ser mayor que cero.
